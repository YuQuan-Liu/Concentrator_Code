...
void param_query(uint8_t * p_buf,uint16_t msg_size){
  ...
  case FN_ALL_READDATA:
    send_meter_data_all(*(p_buf+msg_size));
    break;
  }
}
    ...
    if(cjqmeter_count > 0){
      ...
      for(i=0;i< times_;i++){
        ...
        if(times_==1){//单帧 0
          ...
          p_buf = ack_mulit_header(p_buf,0,(frame_data_len << 2) | 0x03,AFN_QUERY,frame_seq,FN_METER);
        }else{
          if(i==0){//首帧 1
            frame_meter_count = 5;
            frame_data_len = 9+7*frame_meter_count+5+4;
            p_buf = ack_mulit_header(p_buf,1,(frame_data_len << 2) | 0x03,AFN_QUERY,frame_seq,FN_METER);
          }else{
            if(i==times_-1){//尾帧 3
              frame_meter_count = remain;
              frame_data_len = 9+7*frame_meter_count+5+4;
              p_buf = ack_mulit_header(p_buf,3,(frame_data_len << 2) | 0x03,AFN_QUERY,frame_seq,FN_METER);
            }else{//中间帧 2
              frame_meter_count = 5;
              frame_data_len = 9+7*frame_meter_count+5+4;
              p_buf = ack_mulit_header(p_buf,2,(frame_data_len << 2) | 0x03,AFN_QUERY,frame_seq,FN_METER);
            }
          }
        }
